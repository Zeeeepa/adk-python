...
class RuntimeErrorCollector:
    ...
    def get_errors_for_file(self, file_path: str) -> List[RuntimeError]:
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for error in self.runtime_errors:
                errors_by_type[error.context.exception_type] += error.occurrence_count
                errors_by_file[error.file_path] += error.occurrence_count
                errors_by_severity[error.severity] += error.occurrence_count
                errors_by_category[error.category] += error.occurrence_count
            
            return {
                'total_errors': len(self.runtime_errors),
                'total_occurrences': sum(error.occurrence_count for error in self.runtime_errors),
                ...
                'settings': {
                    ...
                    'deduplicate_errors': self.deduplicate_errors
                }
            }
    ...
    def get_recent_errors(self, hours: int = 24) -> List[RuntimeError]:
        """Get errors that occurred within the specified time window."""
        cutoff_time = time.time() - (hours * 3600)
        
        with self._lock:
            return [
                error for error in self.runtime_errors 
        ...
        with self._lock:
            if format.lower() == "json":
                return {
                    ...
                    'errors': [error.to_dict() for error in self.runtime_errors]
                }
...
def create_runtime_error_collector(repo_path: str, **kwargs) -> RuntimeErrorCollector:
    """Create a runtime error collector for a repository."""
    return RuntimeErrorCollector(repo_path, **kwargs)


def collect_runtime_errors_context(repo_path: str, **kwargs):
    """Context manager for runtime error collection."""
    return RuntimeErrorCollector(repo_path, **kwargs)


# Global collector registry for cleanup
_active_collectors: weakref.WeakSet = weakref.WeakSet()


def register_collector(collector: RuntimeErrorCollector):
    """Register a collector for cleanup."""
    _active_collectors.add(collector)


def cleanup_all_collectors():
    """Cleanup all active collectors."""
...
import atexit
atexit.register(cleanup_all_collectors)
